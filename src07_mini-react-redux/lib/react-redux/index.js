 ...
 * 1) react-redux 向外暴露了 2 个 API
    ...
    3) connect 函数
      ...
      StateToProps: 为一个函数, 用来指定向 UI 组件传递哪些一般属性
      mapDispatchToProps: 为一个函数或对象, 用来指定向 UI 组件传递哪些函数属性
      connect()执行的返回值为一个高阶组件: 包装 UI 组件
 */
...
/**
 * connect高阶函数： 接收 mapStateToProps 和 mapDispatchToProps 两个参数，返回一个高级组件函数
...
export function connect (mapStateToProps, mapDispatchToProps) {
  ...
  return (UIComponent) => {
    // 返回容器组件
    return class ContainerComoponent extends React.Component {
      ...
      constructor(props, context) {
        super(props)
        console.log('ContainerComoponent constructor()', context.store)
