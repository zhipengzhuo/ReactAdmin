...
class ProductAddUpdate extends Component {
  ...
  initOptions = async (categorys) => {
    ...
    if (isUpdate && pCategoryId !== 0) {
      ...
      if (subCategorys && subCategorys.length > 0) {
        ...
        // 关联到对应的一级Options上
        // console.log('initOptions', subCategorys, targetOption)
  ...
  // 获取一级或者二级分类列表并显示
  // async的返回值是一个新的promise对象，promise的结果和值由async的结果来决定
  getCategorys = async (parentId) => {
    const result = await reqCategorys(parentId)
    // console.log('getCategorys',parentId, result )
    if (result.status === 0) {
  ...
  render () {
    ...
    // 用来接收级联分类ID的数组
    const categoryIds = []
    // console.log('render', pCategoryId, categoryId, categoryIds)
...
/**
 1. 父组件调用子组件的方法：在父组件中通过ref得到子组件标签对象（也就是组件对象），调用其方法

 2. 子组件调用父组件的方法：将父组件的方法以函数属性的形式传递给子组件，子组件就会调用
 */
